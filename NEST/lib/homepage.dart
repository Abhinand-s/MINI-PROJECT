...
class _MedicineDetailsPageState extends State<MedicineDetailsPage> {
  ...
  void _setReminder() async {
    ...
    _firebaseApi.scheduleNotification(
      'Medicine Reminder',
      'It is time to administer ${widget.medicineName} to ${widget.childName}.',
    );

    // Schedule periodic notifications every 30 minutes
    _firebaseApi.schedulePeriodicNotification(
      'Medicine Reminder',
      'It is time to administer ${widget.medicineName} to ${widget.childName}.',
      Duration(minutes: 30) as RepeatInterval,
    );
...
class _VaccineDetailsPageState extends State<VaccineDetailsPage> {
  ...
  _firebaseApi.scheduleNotification(
    'Vaccine Reminder',
    'It is time to administer ${widget.vaccineName} to ${widget.childName}.',
  );
...
class FirebaseApi {
  ...
  Future<void> schedulePeriodicNotification(String title, String body, RepeatInterval interval) async {
  try {
    const AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails(
      'your_channel_id',
...
class _PersonalPageState extends State<PersonalPage> {
  ...
  Future<void> fetchChildrenData() async {
    ...
    if (user != null) {
      QuerySnapshot querySnapshot = await _firestore.collection('parents').doc(user.uid).collection('children').get();
      setState(() {
        childrenData = querySnapshot.docs.map((doc) => doc.data() as Map<String, dynamic>).toList();
      ...
      body: Column(
        children: <Widget>[
          DropdownButton<String>(
            ...
            items: childrenData.map<DropdownMenuItem<String>>((Map<String, dynamic> child) {
          ...
          if (selectedChildId != null)
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  if (childrenData[index]['id'] == selectedChildId) {
                    return ListTile(
                      title: Text(childrenData[index]['name']),
                      subtitle: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Date of Birth: ${childrenData[index]['dob']}'),
                          Text('Height: ${childrenData[index]['height']} cm'),
                          Text('Weight: ${childrenData[index]['weight']} kg'),
                          Text('Father\'s Name: ${childrenData[index]['father_name']}'),
                          Text('Mother\'s Name: ${childrenData[index]['mother_name']}'),
                          Text('Address: ${childrenData[index]['address']}'),
                        ],
                      ),
                    );
                  }
