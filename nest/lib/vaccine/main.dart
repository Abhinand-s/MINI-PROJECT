...
import 'package:signup_login/vaccine/newentry.dart';
import 'package:sizer/sizer.dart';
import 'package:cloud_firestore/cloud_firestore.dart';
import 'package:signup_login/vaccine/details.dart'; // Import the MedicineDetail page

void main() async {
  WidgetsFlutterBinding.ensureInitialized();
  await Firebase.initializeApp();
  runApp(const MyApp1(childName:'',));
}

class MyApp1 extends StatelessWidget {
    final String childName;
  const MyApp1({Key? key, required this.childName,}) : super(key: key);
    ...
    return Sizer(
      builder: (context, orientation, deviceType) {
        return MaterialApp(
          ...
          home:  Homepage1(childName: childName),
      //     routes: {
      //   '/specificPage': (context) => MyHomePage(title:'Home', childName: childName),
      // },
        );
      },
    );
...
class Homepage1 extends StatelessWidget {
   final String childName;
  const Homepage1({Key? key, required this.childName});
  ...
  Widget build(BuildContext context) {
   ...
   appBar: AppBar(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
             TopContainer(childName:childName),
            SizedBox(height: 2.h),
           Flexible(child: BottonContainer(childName:childName,))
          ],
        ),
      ),
      floatingActionButton: InkResponse(
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => NewEntryUI(childName: childName)),
          );
        },
        child: SizedBox(
          width: 18.w,
...
class TopContainer extends StatelessWidget {
  final String childName;
  const TopContainer({Key? key, required this.childName, });
    ...
    return Column(
      ...
      children: [
        ...
        SizedBox(
         ...
         MedicineCount(childName:childName), // Widget to show the number of medicines
      ],
    );
  }
}
...
class BottonContainer extends StatelessWidget {
  final String childName;
  const BottonContainer({Key? key, required this.childName});

  @override
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('medicine').where('childName', isEqualTo: childName).snapshots(),
...
class MedicineCount extends StatelessWidget {
  final String childName;
  const MedicineCount({Key? key, required this.childName});
